...
static inline bool lineOfSight(int from, int to)
{
	int x0 = conX(from), y0 = conY(from);
	int x1 = conX(to), y1 = conY(to);
	...
	if(dx>=dy)
	{
		while(x0!=x1)
		{
			...
			if(f>=dx)
			{
				if(w2map[con(x0 + ((sx - 1)/2), y0 + ((sy - 1)/2))])
					return false;
				y0+=sy;
				f-=dx;
			}
			if(f!=0 && w2map[con(x0 + ((sx - 1)/2), y0 + ((sy - 1)/2))])
				return false;
			if(dy==0 && w2map[con(x0 + ((sx - 1)/2), y0)] && w2map[con(x0 + ((sx - 1)/2), y0 - 1)])
				return false;
			x0+=sx;
		}
	}
	else
	{
		while(y0!=y1)
		{
			...
			if(f!=0 && w2map[con(x0 + ((sx - 1)/2), y0 + ((sy - 1)/2))])
				return false;
			if(dx==0 && w2map[con(x0, y0 + ((sy - 1)/2))] && w2map[con(x0 - 1, y0 + ((sy - 1)/2))])
				return false;
			y0+=sy;
		}
	}
...
static double thetaStar(int start, int goal)
{
	...
	while (!openList.empty())
	{
		...
		for(int i=1; i<=8; i++)
		{
			...
			if(!closedList[next])
			{
				...
				if(w2map[start]==0 && w2map[goal]==0 && lineOfSight(parent[curr], next))//path 2
				{
					double newgscore = gscores[parent[curr]] + getCcost(parent[curr], next, i);
					if(newgscore < oldgscore)
					{
						parent[next] = parent[curr];
